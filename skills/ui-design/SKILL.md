---
name: ui-design
description: "Use for visual UI design engineering: brand systems, themes, layout/aesthetic implementation, component styling, page visual polish, prototypes, dashboards, slides, animations, and browser-rendered presentation work. Existing UX friction where the page works but is hard to use belongs to usability; new visible capabilities/actions/data/API/state flow belong to develop; broken behavior/errors belong to debug. Not for back-end, CLI, or non-visual coding tasks."
---
...
## Cynos Practice Contract

When this skill is used by the `ui-design` practice, keep the upstream web-design-engineer workflow intact and add this evidence contract:

- Start with `cynos_start_work(practice="ui-design")` once routing is clear.
- Use `cynos_ask_user` after work starts for design-direction, v0, brand, or durable design-doc decisions that affect the deliverable. Ask one design decision per `cynos_ask_user` call. If the user delegated or already provided enough context, record that instead of asking mechanically.
- Record `uiDesign.directionDecision.source` as one of `user-confirmed`, `user-delegated`, `provided-spec`, `existing-brand-spec`, or `small-tweak`. If you choose `user-confirmed`, there must be a real captured user answer; do not claim confirmation from ordinary chat after work has started.
- Read or write root `brand-spec.md` according to the selected foundation mode. It is a durable design foundation, not a task diary.
- Write final UI deliverables as real in-project product files. Do not list screenshots, `.cynos/`, `.playwright-cli/`, `/tmp`, scratch/cache files, or test specs in `uiDesign.implementation.artifacts[]`.
- Verify the final rendered UI in a browser after the last UI-like production write. Build/typecheck/test output does not replace browser evidence.
- Finish with critique or evidenced user confirmation, including how the implementation aligns with the design foundation.
- Finish with `cynos_check_completion({ completionEvidence })`; do not claim completion until the gate passes.
...
When the request mentions a specific product, brand, technology, SDK, or event you're not sure about, use `cynos_search` to verify the current facts from authoritative sources before designing around them. Never assert unstable facts from memory.
...
Complete this lightweight self-check before delivery. In Cynos's `ui-design` practice, also capture direct browser evidence after the final UI-like write; the full acceptance harness remains opt-in unless the user explicitly requests it:

- [ ] **Step 0 ran** if any specific product/brand was named — facts verified via `cynos_search`, not assumed
...
This skill is used by the `ui-design` practice. Keep the upstream workflow intact. Cynos does **not** replace the web-design-engineer workflow with a second checklist; it only checks the final engineering evidence.

Before implementation, consult existing project context when it affects the design work:

- `PROJECT.md` for design-system, brand, frontend architecture, and durable project rules;
- `docs/testing.md` for build/browser/surface-verification verification commands;
- root `brand-spec.md` if it exists.

`PROJECT.md` / `docs/testing.md` are project knowledge, not a completion hard gate; do not fabricate reads for tiny unambiguous local tweaks. `brand-spec.md` remains evidence-gated when the selected foundation mode depends on it.

### When to ask the user

Keep the upstream Step 3 / Step 4 checkpoints. When you say you will wait, actually wait.

Ask and wait for confirmation when:

- creating a new page, landing page, prototype, slide deck, dashboard, or major layout direction;
- creating or substantially updating `brand-spec.md`;
- design direction is unclear and the user gave no brand/reference/style;
- a v0 direction will affect a large amount of later build work.

You may proceed without more questions when:

- the user explicitly says "you decide", "don't ask", "use your judgment", or equivalent;
- the user provides a complete PRD + brand-spec + clear output target;
- the task is a small tweak within an existing design system.

Small tweaks may skip v0 only when they are local visual changes such as one button color, spacing, or hover state; no new page/layout structure; no brand token changes. Record `directionDecision.source='small-tweak'`. Still run browser verification and critique.

### `brand-spec.md` handling

`brand-spec.md` belongs at the project root so future UI/design work can find it automatically.

Use these modes in completion evidence:

- `brand-spec-existing`: root `brand-spec.md` already exists; read and follow it. Do not rewrite it just to log the current task.
- `brand-spec-created` / `brand-spec-updated`: use for new brand work, new durable design tokens, or new durable brand assets. Write root `brand-spec.md` and explain why.
- `design-system-only`: use only for non-branded one-off prototypes or when the user explicitly does not want durable brand documentation. Explain why no `brand-spec.md` was written.

Do not put `brand-spec.md` in `docs/` or `assets/`. Do not turn it into a task diary.

For branded work, never fake assets: no colored rectangle logo, no CSS silhouette pretending to be product imagery, no hand-drawn SVG substitute for missing official assets. If a required logo/product/UI asset cannot be sourced after a real attempt, stop and ask the user or mark the asset as pending in `brand-spec.md` if the user allows placeholders.

### Completion evidence intent
...
- `uiDesign.directionDecision` records how the design direction was chosen:
  - `source`: `user-confirmed` | `user-delegated` | `provided-spec` | `existing-brand-spec` | `small-tweak`;
  - `summary`: the concrete direction/brand/v0/tweak basis;
  - if `source='user-confirmed'`, use `cynos_ask_user` and ask one design decision per question. The checkpoint only proves a user answer exists; smoke/review judges whether it corresponds to the decision.
- `uiDesign.foundation` records brand/design foundation:
  - mode, summary;
  - root `brandSpecPath` when using brand-spec;
  - `visualDirection` and reason when using `design-system-only`;
  - optional `tokensSummary` and `assetPaths`.
- `uiDesign.implementation.summary` summarizes the implementation.
- `uiDesign.implementation.artifacts[]` lists only final in-project UI deliverables. Each listed path must have real `write`/`edit` evidence. Do not list scratch demos, temporary downloads, old v0 files replaced later, screenshots, browser artifacts, `/tmp` files, `.cynos/`, `.playwright-cli/`, test specs, e2e specs, or runner config here.
- `uiDesign.designFidelity` records the final alignment self-audit when implementation writes exist: `foundationUsed`, `alignmentSummary`, and optional `deviations[]` with reasons.
- `browserVerification` records real browser/screenshot/console evidence after the last UI-like production write. ui-design does not accept browser not-applicable, `vite build`, `tsc`, or `npm test` as a substitute for browser evidence.
- `critique` records self-critique, or `confirmation` records user approval. If you use `confirmation`, it must correspond to a real user answer or explicit authorization in the original prompt. Include design-foundation alignment in `uiDesign.designFidelity` rather than relying on prose only.
- `verification.summary` describes the real successful verification command.
- `finalization` records verification, git status, and local commit status for this UI change.

Optional `toolCallId` fields are only for explicit references; leave them out rather than guessing.

### Browser blocked behavior

Browser evidence is mandatory for ui-design completion and must be captured after the final UI-like product edit. Prefer the `browser-automation` skill / Playwright CLI for the lightweight headless path.

If browser startup fails because of missing system dependencies:

1. Try at most two Playwright CLI evidence actions, such as `snapshot` and `screenshot`/`console`. Project browser/e2e commands may be extra verification, but they do not replace Playwright CLI direct browser evidence.
2. Do not keep downloading browsers, run `install-deps` repeatedly, use sudo, or try hacks such as `LD_PRELOAD` unless the user explicitly authorizes them.
3. Do not use `vite build` / `tsc` / `npm test` as a substitute for browser evidence.
4. Ask the user whether to switch environment, authorize dependency installation, or stop as blocked. A blocked browser is not a successful ui-design completion.

After successful verification, follow the Cynos local commit policy: commit this UI change locally unless the user explicitly opted out. Never push/tag/publish/deploy in ui-design. If `git commit` fails, record `commit.status='failed'` with the real reason; do not bypass hooks or retry blindly.

Finish with `cynos_check_completion({ completionEvidence })`; do not claim the UI work is complete until it passes.
