...
When the user asks "review this", "is it good?", "score this", "how does it look?", or you want to do a self-check before declaring done, run a **5-dimension critique**:
...
- [ ] Browser console shows **no errors, no warnings** (use `browser-automation` / `npx --yes @playwright/cli console`)
- [ ] Renders correctly on **target devices/viewports** (use `browser-automation` snapshots/screenshots; responsive web → mobile / tablet / desktop; mobile prototype → target device; slide decks/video with fixed dimensions → scaling container adapts without distortion)
